...
public class Variant {
  ...
  private static Optional<State> inferState(HttpServletRequest request, Session ssn) {
    ...
    String referrerPath = Optional.ofNullable(request.getHeader("referer"))
      .map(refererString -> {
        try { return new URL(refererString).getPath(); }
    ...
    return ssn.getSchema().getStates().stream()
  ...
  public static Boolean isExperienceLive(StateRequest stateRequest, String experimentName, String experienceName) {
    return stateRequest
      .getLiveExperience(experimentName)
